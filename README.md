# Lab 56 – Suspicious Process Investigation with Wazuh
...
|---|---|---|
| Wazuh Agent | Agent 001 Active | Endpoint visibility confirmed |
...
| PowerShell Signature | Valid | Signed executable |
| Script Path | C:\Users\Public | Requires investigation |
| Execution Policy | Bypass | Suspicious indicator |
| Script Contents | Harmless output commands | Benign laboratory artifact |
| Script Hash | SHA256 recorded | Artifact identification |
| Child Process | cmd.exe | Relevant process activity |
| Sysmon | Event ID 1 | Process creation telemetry |
| Security | Event ID 4688 | Process creation telemetry |
| Wazuh | Agent telemetry | Centralized correlation |

---

# Risk Assessment

The following indicators were considered suspicious:
...
However, the following evidence provides strong benign context:
...
The activity was intentionally generated as part of Lab 56.

The suspicious indicators were still valid reasons for investigation, but the complete context supports a benign laboratory disposition.
...
1. Wazuh Agent 001 was active.
2. The investigated endpoint was `DESKTOP-9MMM37V`.
3. The associated user was `desktop-9mmm37v\dell`.
4. PowerShell was located at the expected Windows system path.
5. The PowerShell executable had a valid Authenticode signature.
6. `Lab56-Test.ps1` was created in `C:\Users\Public`.
7. The script contained only harmless `Write-Output` statements.
8. PowerShell was executed with `-ExecutionPolicy Bypass`.
9. A `cmd.exe` child process was intentionally created.
10. Sysmon Event ID 1 confirmed `cmd.exe` process creation.
11. Windows Security Event ID 4688 was identified as an additional process-creation source.
12. The script SHA256 was recorded.
13. Wazuh was used to correlate endpoint telemetry.
14. The available evidence does not demonstrate malware execution.

---

# Lessons Learned

A legitimate Windows executable can still be involved in suspicious-looking activity.

`ExecutionPolicy Bypass` should be treated as an investigation indicator rather than an automatic malicious verdict.

Process-tree analysis provides valuable context when investigating PowerShell and CMD activity.

File metadata, file contents, hashes, signatures, Windows event logs, Sysmon, and Wazuh should be correlated before making a final decision.

Most importantly:

> Suspicious does not automatically mean malicious.

A SOC analyst should investigate the execution context and make a defensible disposition based on the available evidence.
